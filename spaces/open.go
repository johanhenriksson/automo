...
func setupTabs(session, workdir string, tabs []config.Tab) error {
	for i, tab := range tabs {
		windowIndex := strconv.Itoa(i)

		if i == 0 {
			// First tab uses the default window (index 0)
			if tab.Name != "" {
				if err := tmux.RenameWindow(session, windowIndex, tab.Name); err != nil {
					return err
				}
			}
		} else {
		...
		if tab.Cmd != "" {
			if err := tmux.SendKeys(session, windowIndex, tab.Cmd); err != nil {
				return err
			}
		}
	}
	...
	return tmux.SelectWindow(session, "0")
}
